...
def index(request):
    responseDolar = requests.get('https://api.sbif.cl/api-sbifv3/recursos_api/dolar?apikey=4bf7fef2e83e5be2125b3631dc513ad6b9538331&formato=JSON')
    responseUF = requests.get('https://api.sbif.cl/api-sbifv3/recursos_api/uf?apikey=4bf7fef2e83e5be2125b3631dc513ad6b9538331&formato=JSON')
...
def monthRange(request):
    ...
    if yearRange1 <= yearRange2:
        ...
        responseDolar = requests.get('https://api.sbif.cl/api-sbifv3/recursos_api/dolar/periodo/%s' % finalRange, params=payload)
        responseUF = requests.get('https://api.sbif.cl/api-sbifv3/recursos_api/uf/periodo/%s' % finalRange, params=payload)
        dolarData = responseDolar.json()
        ufData = responseUF.json()
        return render(request, 'apitest/index.html', {
